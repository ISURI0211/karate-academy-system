...
import {
  ...
  FaGraduationCap, FaMoneyBillWave, FaTrophy,
  FaBook, FaSignOutAlt, FaChevronLeft, FaChevronRight, FaChartLine
} from 'react-icons/fa';
...
const navigationGroups = [
  {
    title: 'Overview',
    items: [
      { name: 'Student Hub', icon: FaTachometerAlt, href: '/dashboard/student-dashboard' },
    ]
  },
  {
    title: 'Training',
    items: [
      { name: 'Class Schedule', icon: FaCalendarAlt, href: '/dashboard/student-dashboard/classes' },
      { name: 'Attendance History', icon: FaClipboardCheck, href: '/dashboard/student-dashboard/attendance' },
      { name: 'Belt Progression', icon: FaGraduationCap, href: '/dashboard/student-dashboard/progress' },
      { name: 'Smart Progress Tracking', icon: FaChartLine, href: '/dashboard/student-dashboard/smart-progress' },
    ]
  },
  {
    title: 'Academy & Finance',
    items: [
      { name: 'Fees & Payments', icon: FaMoneyBillWave, href: '/dashboard/student-dashboard/fees' },
      { name: 'Events', icon: FaTrophy, href: '/dashboard/student-dashboard/events' },
      { name: 'Resources', icon: FaBook, href: '/dashboard/student-dashboard/resources' },
    ]
  }
];
...
export default function StudentSidebar() {
  ...
  const ToriiIcon = () => (
    <svg className="w-5 h-5 text-sky-600 transition-transform duration-300 group-hover:scale-110" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
      <path d="M4 6h16" />
      <path d="M6 6v14" />
      <path d="M18 6v14" />
      <path d="M8 11h8" />
      ...
      <div className={`flex items-center gap-3 px-6 py-6 border-b border-slate-100/80 ${collapsed ? 'justify-center px-2' : ''}`}>
        ...
        {!collapsed && (
          <div className="flex flex-col">
            ...
            <span className="text-[9px] font-bold uppercase tracking-[0.15em] text-sky-600 mt-1 leading-none">Student Portal</span>
          </div>
        )}
      </div>
          ...
          <div key={group.title} className="space-y-2">
            ...
            <nav className="space-y-2">
              {group.items.map((item) => {
                const isActive = pathname === item.href || (item.href !== '/dashboard/student-dashboard' && pathname?.startsWith(item.href));
                ...
                return (
                  ...
                  >
                    {isActive && (
                      <span className="absolute left-0 top-1/2 -translate-y-1/2 w-1.5 h-5 bg-sky-500 rounded-r-full"></span>
                    )}
                    <Icon className={`flex-shrink-0 text-base transition-colors duration-200 ${isActive ? 'text-sky-600' : 'text-slate-400 group-hover:text-slate-600'}`} />
          ...
          <div className="flex items-center gap-3">
            ...
            {!collapsed && (
              <div className="min-w-0">
                ...
                <p className="text-[9px] text-slate-400 font-semibold tracking-wider uppercase mt-0.5">Student</p>
              </div>
            )}
          </div>
  ...
  return (
    <>
      ...
      <button
        id="student-mobile-sidebar-toggle"
        className="hidden"
