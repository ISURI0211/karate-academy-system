...
import { 
  ...
  FaCheckCircle, FaUserCheck, FaUserTimes, FaExclamationTriangle,
  FaTable, FaListUl, FaSearch, FaInfoCircle
} from 'react-icons/fa';
...
export default function AdminAttendancePage() {
  ...
  // Sync date picker with selected month when month changes if date falls outside
  useEffect(() => {
  ...
  // Quick mark all students present
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h2 className="text-xl font-bold tracking-tight text-slate-800 flex items-center gap-2">
            <FaClipboardCheck className="text-red-600" /> Attendance Ledger
          </h2>
          <p className="text-xs text-slate-500 mt-1">Manage daily roll call sheets and view monthly attendance matrices</p>
        </div>
      ...
      <div className="bg-white p-5 rounded-xl border border-slate-100 shadow-[0_2px_8px_-3px_rgba(0,0,0,0.02)] grid grid-cols-1 md:grid-cols-3 gap-4 items-start">
        ...
        <div>
          ...
          <input
            ...
            className="w-full px-3 py-2 bg-slate-50 border border-slate-200/60 rounded-xl text-xs font-semibold text-slate-800 focus:outline-none"
          />
        </div>
      ...
      {activeTab === 'daily' && (
        loadingRoster ? (
          <div className="p-12 bg-white rounded-xl border border-slate-100 flex flex-col items-center justify-center text-slate-400 gap-3">
            ...
            <span className="text-xs font-semibold">Retrieving class roll...</span>
          </div>
        ) : roster.length > 0 ? (
          <div className="grid grid-cols-1 lg:grid-cols-4 gap-6 items-start">
            ...
            <div className="bg-white rounded-xl border border-slate-100 shadow-[0_2px_8px_-3px_rgba(0,0,0,0.02)] overflow-hidden lg:col-span-3">
              <div className="p-4 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
                ...
                <button
                  ...
                  className="text-[10px] font-bold text-sky-600 hover:text-sky-850 transition-colors"
                >
                  Mark All Present
                </button>
              </div>
                      ...
                      <tr key={student.student_id} className="hover:bg-slate-50/30 transition-colors">
                        ...
                        <td className="px-6 py-3">
                          <div className="flex items-center gap-3">
                            <div className="w-8.5 h-8.5 rounded-xl bg-slate-150 text-slate-700 font-bold text-xs flex items-center justify-center flex-shrink-0">
                            ...
                            <div>
                              ...
                              <span className="text-[11px] font-mono font-extrabold text-indigo-700 bg-indigo-50 px-2 py-0.5 rounded-md border border-indigo-200 shadow-sm inline-block mt-1">
                        ...
                        {/* Belt rank */}
                        <td className="px-6 py-3">
                          <span className={`text-[9px] font-extrabold px-2.5 py-0.5 rounded-full border ${BELT_COLORS[student.belt_rank] || 'bg-slate-100 border-slate-200'}`}>
                            {student.belt_rank}
                          </span>
                        </td>
              ...
              <div className="p-4 border-t border-slate-100 flex justify-end">
                <button
                  ...
                  disabled={saving}
                  className="px-6 py-2 bg-slate-900 hover:bg-slate-800 disabled:opacity-60 text-white rounded-xl text-xs font-semibold shadow-sm transition-colors flex items-center gap-2"
                >
            ...
            {/* Quick Statistics Sidebar panel */}
            <div className="bg-white border border-slate-100 p-5 rounded-xl shadow-[0_2px_8px_-3px_rgba(0,0,0,0.02)] space-y-4">
              <h3 className="text-xs font-bold text-slate-800 border-b border-slate-50 pb-2 flex items-center gap-2">
                <FaClipboardCheck className="text-slate-400" /> Roster Sheet Stats
              </h3>
              ...
              <div className="space-y-3.5">
                <div className="flex justify-between items-center text-xs">
                  ...
                  <span className="font-extrabold text-slate-700 bg-slate-100/70 px-2 py-0.5 rounded">{markedCount.present}</span>
                </div>
                <div className="flex justify-between items-center text-xs">
                  ...
                  <span className="font-extrabold text-slate-700 bg-slate-100/70 px-2 py-0.5 rounded">{markedCount.absent}</span>
                </div>
                <div className="flex justify-between items-center text-xs">
                  ...
                  <span className="font-extrabold text-slate-700 bg-slate-100/70 px-2 py-0.5 rounded">{markedCount.excused}</span>
                </div>
                <div className="flex justify-between items-center text-xs pt-2 border-t border-slate-50">
                  ...
                  <span className="font-extrabold text-red-600 bg-red-50 border border-red-100 px-2 py-0.5 rounded">{markedCount.unmarked}</span>
                </div>
              </div>
      ...
      {activeTab === 'monthly' && (
        loadingMonthly ? (
          <div className="p-12 bg-white rounded-xl border border-slate-100 flex flex-col items-center justify-center text-slate-400 gap-3">
            <FaSpinner className="animate-spin text-lg" />
            <span className="text-xs font-semibold">Generating monthly attendance matrix...</span>
          </div>
        ) : (
            ...
            {filteredMonthlyStudents.length > 0 ? (
              <div className="bg-white rounded-xl border border-slate-100 shadow-[0_2px_8px_-3px_rgba(0,0,0,0.02)] overflow-x-auto">
                <table className="w-full text-left border-collapse text-xs">
                  <thead>
                    <tr className="border-b border-slate-100 text-[10px] font-extrabold uppercase tracking-wider text-slate-400 bg-slate-50/60">
                      ...
                      return (
                        <tr key={student.student_id} className="hover:bg-slate-50/40 transition-colors">
                          ...
                          <td className="px-4 py-3 sticky left-0 bg-white shadow-[1px_0_0_0_#f1f5f9] z-10">
                            ...
                            <span className="text-[11px] font-mono font-extrabold text-indigo-700 bg-indigo-50 px-1.5 py-0.5 rounded border border-indigo-200 inline-block mt-1">
                              ID: #{student.student_id}
                            </span>
                          </td>
                          ...
                          <td className="px-3 py-3">
                            <span className={`text-[8px] font-extrabold px-2 py-0.5 rounded-full border ${BELT_COLORS[student.belt_rank] || 'bg-slate-100 border-slate-200'}`}>
                              {student.belt_rank}
                            </span>
                          </td>
