...
function StudentSmartProgressContent() {
  ...
  const fetchProgressData = async (p: string) => {
    try {
      setLoading(true);
      setError('');
      const res = await fetch(`/api/student/smart-progress?period=${p}`);
      const d = await res.json();
      if (d.success) {
        setData(d);
      } else {
        setError(d.error || 'Failed to retrieve smart progress analytics.');
      }
    } catch (err) {
  ...
  const handlePeriodChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    router.push(`/dashboard/student-dashboard/smart-progress?period=${val}`);
  };
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center py-32 gap-4 text-slate-400">
        ...
        <p className="text-xs font-semibold">Computing Your Smart Progress Analytics...</p>
      </div>
    );
  }
  ...
  if (error || !data || !data.mlAnalysis || !data.student) {
    return (
      <div className="space-y-4 w-full max-w-7xl mx-auto">
        ...
        >
          <FaArrowLeft size={10} /> Back to Student Hub
        </Link>
        <div className="p-6 bg-rose-50 border border-rose-100 rounded-2xl text-rose-700 text-xs font-semibold">
          {error || 'No progress analytics available.'}
        </div>
      </div>
    );
  }

  const { student, mlAnalysis } = data;
    ...
    <div className="space-y-5 w-full">
        ...
        <div className="flex items-center gap-4 flex-wrap">
          ...
          <div>
            ...
            <p className="text-[11px] text-slate-500 font-medium mt-0.5 flex items-center gap-1.5">
              <FaChartLine className="text-sky-600" /> My Karate Training Progress & Skill Development
            </p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 items-start">
        ...
        <div className="space-y-6">
          ...
          {/* CARD: TRAINING RECOMMENDATION (MODERN HIGH-IMPACT STUDENT AI THEME) */}
          <motion.div
            initial={{ opacity: 0, y: 10 }}
            animate={{ opacity: 1, y: 0 }}
            ...
            <div className="space-y-4 text-xs leading-relaxed text-slate-300">
              ...
              <div className="space-y-1.5 pt-1">
                <p className="font-black text-sky-400 uppercase tracking-widest text-[10px] flex items-center gap-1.5">
                  <FaBullseye className="text-rose-400" /> Recommended Training Focus:
                </p>
...
export default function StudentSmartProgressPage() {
  return (
    <Suspense fallback={
      <div className="flex flex-col items-center justify-center py-32 gap-4 text-slate-400">
        <FaSpinner className="animate-spin text-2xl text-sky-600" />
